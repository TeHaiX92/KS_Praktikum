...
class TcpLayer {
	...
	void receive() {
		...
		while (run) {
			...
			if(ownPort == t_pdu.dstPort) {
				...
				switch(true) {
					case (recvRstFlag):
						Utils.writeLog("TcpLayer", "reset", "Verbindung wird abgebrochen", 2);
						fsm.currentState = State.S_IDLE
						break
					case (recvSynFlag && !recvAckFlag):
						Utils.writeLog("TcpLayer", "Case:", "SynFlag && !AckFlag", 2)
						event = Event.E_RCVD_SYN
						break
					case (recvFinFlag):
						Utils.writeLog("TcpLayer", "Case:", "FinFlag", 2)
						event = Event.E_RCVD_FIN
						break
					case (recvSynFlag && recvAckFlag):
						Utils.writeLog("TcpLayer", "Case:", "SynFlag && AckFlag", 2)
					...
					case (recvAckFlag && t_pdu.sdu.size() == 0):
						Utils.writeLog("TcpLayer", "Case:", "AckFlag  && t_pdu.sdu.size() == 0", 2)
				...
				if (event) {
					...
					Utils.writeLog("TcpLayer", "event:", "${event}", 2)
					handleStateChange(event)
				}
			} else {
				Utils.writeLog("TcpLayer", "error",
					"Zielport ${t_pdu.dstPort} entspricht nicht dem eigenen Port ${ownPort}", 2)
			}
		}
	}
	...
	void send() {
		...
		while (run) {
			...
			switch (at_idu.command) {
				case OPEN: // Neue Verbindung öffnen
				...
				case CLOSE: // Verbindung schließen

					handleStateChange(Event.E_DISCONN_REQ)
					break

				case DATA: // Daten senden

					int dataLength = at_idu.sdu.length()
					int start = 0
					int end = MSS

					// ist die SDU kleiner als die MSS, dann komplett übertragen
					if(dataLength <= MSS){
						sendData = at_idu.sdu // Anwendungsdaten übernehmen
						handleStateChange(Event.E_SEND_DATA)
					}
					// ist die SDU größer, dann SDU aufsplitten und Segmente senden
					else {
						while(start < at_idu.sdu.length()){
							// Sende Segment
							sendData = at_idu.sdu[start..end]
							Utils.writeLog("TcpLayer", "send", "Segment: Laenge:${sendData.length()}, " + // sendData.length() = MSS
								"Byte:${sendData.bytes.size()} Gesamtlaenge:${dataLength}, " +
								"Gesamtbytes:${at_idu.sdu.bytes.size()} Daten:${sendData}", 2)

							// naechstes Segment mit MSS
							start += MSS
							end += MSS
							if (end > at_idu.sdu.length()) {
								end = at_idu.sdu.length()-1
							}
							handleStateChange(Event.E_SEND_DATA)
						}
					}
	...
	synchronized void handleStateChange(int event) {
		...
		Utils.writeLog("TcpLayer", "hSC", "${nextState}", 2)
		if (nextState) {
			// Neuen Zustand behandeln
			...
			// Daten empfangen
				...
				case (State.S_RCVD_DATA):
					...
					Utils.writeLog("TcpLayer", "hSC", "Achtung!Achtung!", 2)
			...
			// ----------------------------------------------------------
				...
				default:
					...
					Utils.writeLog("TcpLayer", "hSC:", "default-Case", 2)
					break
			}
		}
	}
